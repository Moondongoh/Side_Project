# week4
# HSV를 이용하여 입력 이미지에서 빨간색 선을 탐지하고, 결과를 시각화하는 코드입니다.
...
LOG_FILE = LOG_DIR / "red_line_detection.log"
...
class RedLineDetector:
    ...
    def process_image(
        ...
        if visualize:
            ...
            if save_result:
                if output_path is None:
                    output_path = str(OUTPUT_DIR / f"red_line_result_{timestamp}.jpg")
        ...
        # JSON 결과 저장
        if save_result:
            json_path = str(OUTPUT_DIR / f"red_line_result_{timestamp}.json")
            json_result = {
                k: v
