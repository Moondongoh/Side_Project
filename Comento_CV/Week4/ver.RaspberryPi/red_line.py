                ...
                if self.path == "/stream.jpg":
                    if self.server.streamer.latest_frame is not None:
                        ...
                        with self.server.streamer.frame_lock:
                            self.wfile.write(self.server.streamer.latest_frame)
                    else:
...
class RedLineFollowingCar:
    def __init__(self, camera_index=0, headless=True, enable_streaming=True):
        ...
        logger.info("빨간색 선 추적 자동차 초기화 완료")
        logger.info("빨간색 테이프나 선을 바닥에 붙여주세요!")
    ...
    def detect_red_line(self, img):
        ...
        # 시각화를 위한 이미지 복사
        vis_img = img.copy() if self.enable_streaming else None

        # 빨간색 영역의 무게중심 찾기
        moments = cv.moments(red_mask)
        line_detected = False
        red_center_x = width // 2  # 기본값

        if moments["m00"] > 500:  # 충분한 빨간색 픽셀이 있는 경우
    ...
    def calculate_line_center(self, red_center_x, line_detected):
        ...
        if len(self.line_history) >= 3:
            weights = [0.2, 0.3, 0.5]  # 최근 값에 더 큰 가중치
            smoothed_center = np.average(list(self.line_history)[-3:], weights=weights)
        else:
    ...
    def control_car(self, line_center_x, img_width):
        ...
        if self.motor is None:
            direction_prefix = "SIMULATION: "
        else:
            direction_prefix = ""

        # 선을 너무 오래 놓쳤으면 정지
        if self.lost_count > 30:  # 1초 정도
            if self.motor:
                self.motor.stop()
    ...
    def add_visualization(self, img, direction, error, line_center_x):
        ...
        # 중앙선 표시
        cv.line(
            img, (width // 2, 0), (width // 2, height), (255, 0, 0), 2
        )  # 화면 중앙 (파란선)
        cv.line(
    ...
    def log_status(self, direction, error, line_center_x):
        ...
        if current_time - self.stats["last_log_time"] > 2.0:  # 2초마다 로그
    ...
    def run(self):
        ...
        # 웹 스트리밍 시작
        if self.enable_streaming:
            self.stream_thread = threading.Thread(target=self.streamer.start_server)
...
def test_red_detection(image_path):
    ...
    car = RedLineFollowingCar(headless=True, enable_streaming=False)
...
logging.basicConfig(
    ...
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler("/tmp/red_line_following.log"),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "test":
            image_path = sys.argv[2] if len(sys.argv) > 2 else "/tmp/test_image.jpg"
            test_red_detection(image_path)
        elif sys.argv[1] == "no-stream":
            car = RedLineFollowingCar(
                camera_index=0, headless=True, enable_streaming=False
